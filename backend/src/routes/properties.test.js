...
describe('Properties API', () => {
    ...
    describe('GET /api/properties', () => {
        test('returns properties with pagination', async () => {
            pool.query
                ...
                .mockResolvedValueOnce([[{ ListingId: '123', City: 'Portland' }]]); // data query
            ...
            expect(response.body.results[0].City).toBe('Portland');
        });
        ...
        test('filters by price range', async () => {
            pool.query
                ...
                .mockResolvedValueOnce([[{ ListingId: '123', ListPrice: 400000 }]]);
            
            const response = await request(app)
                .get('/api/properties?minPrice=300000&maxPrice=500000')
                .expect(200);

            expect(response.body.results[0].ListPrice).toBe(400000);
        });
    });
    ...
    describe('GET /api/properties/:id', () => {
        test('returns property by id', async () => {
            ...
            const response = await request(app)
                ...
                .expect(200);
            expect(response.body.ListingId).toBe('123');
        });
    ...
    describe('GET /api/properties/:id/openhouses', () => {
        test('returns open houses for property', async () => {
            pool.query
                .mockResolvedValueOnce([[{ ListingId: '123' }]]) // property check
